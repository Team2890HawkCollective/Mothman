...
public class TargetingSubsystems extends SubsystemBase {
    ...
    public Command aimAtHubPose(SwerveSubsystem swerveDrive, CommandXboxController driverXbox) {
        return new RunCommand(() -> {
            ...
            double angleSpeed = photonAimPIDController.calculate(currentRobotPose.getRotation().getRadians(),
                    Constants.TargetingConstants.allianceHubPose.getRotation().getRadians());                     //calculate the PIDs and curve needed to reach hub angle
            ...
            swerveDrive.drive(new Translation2d(driverXbox.getLeftY()/2, driverXbox.getLeftX()/2), angleSpeed,
                    true);                                                                              //the driverXbox.getLeftY() and getleftX() is to allow 
        }, swerveDrive);                                                                                              //driver to drive but not rotate while we auto aim, but we cut speed in half
    }
    ...
    } //stops auto aim in autonomous mode with the .until
    ...
    public static void getHubPoseTheta(SwerveSubsystem swerveDrive) {
        if (alliance.isPresent()) {
            ...
            //putting the desired angle our robot needs to rotate to into our hubpose z value so that we can calculate differenc in autoaim method above
        }
    ...
    public void periodic() {
        alliance = DriverStation.getAlliance(); //checks alliance based off driverstation
